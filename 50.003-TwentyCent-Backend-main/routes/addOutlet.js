...
router.post('/', async function(req,res) {
    ...
    if (!utils.checkRequestCompleteness(req,["sessionId","institutionId","address"])) {
    ...
    var result = await db.makeTransaction(req,async (dbo,auth)=>{
        ...
        queryInt = await db.performTransactionStep(dbo,`INSERT INTO t4 (t1_id,address,checklist) VALUES (${req.body["institutionId"]},\'${req.body["address"]}\',\'{}\') RETURNING id;`);
        if (queryInt.length===0) throw false;

        return {"outletId": queryInt[0]["id"]};
    });
